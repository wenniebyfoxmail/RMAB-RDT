"""
NHGP-based Transition Matrix Builder (v3.1)
============================================

This module constructs the time-homogeneous transition matrix P̄ from NHGP
(Non-Homogeneous Gamma Process) degradation model.
...
Design Decision (v3.1):
- recovery_prob = 0.02 (default) to prevent absorbing state collapse
- This models "external maintenance intervention" where 2% of worst-state
  road segments receive major repairs and recover to best state
- Without recovery_prob > 0, all arms eventually reach s=h=4 and AoII=0
"""
...
class NHGPParams:
    ...
    """
    c: float = 0.8       # Degradation rate coefficient [SOURCE: Synthetic]
...
def get_default_nhgp_classes(J: int = 5,
                              recovery_prob: float = 0.02,  # Required for non-zero AoII
    ...
    Args:
        ...
        recovery_prob: Recovery probability from worst state to best state.
                      Default 0.02 (2%) models external maintenance intervention.
                      This is REQUIRED to prevent absorbing state (AoII=0).

    Returns:
        List of (name, P_bar, c_ratio) tuples

    Note:
        recovery_prob > 0 breaks strict upper triangular structure but is
        necessary for meaningful AoII dynamics. Can be justified as
        "probabilistic major road repair/reconstruction".
    """
    ...
    for (name, P_bar), ratio in zip(classes, [1.0, 2.0]):
        # Add recovery probability to prevent absorbing state collapse
        # This models external maintenance intervention (e.g., major road repair)
        # recovery_prob=0.02 means 2% chance of recovering from worst state to best state
        if recovery_prob > 0:
            J_local = P_bar.shape[0]
            P_bar[J_local-1, J_local-1] = 1.0 - recovery_prob
            P_bar[J_local-1, 0] = recovery_prob  # Recover to state 0 (best state)
