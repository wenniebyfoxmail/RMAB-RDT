...
from config import (
    ...
    ArmClassConfig, get_nhgp_arm_classes, get_ontario_arm_classes,
    get_channel_params, R_TABLE, compute_control_cost
)
...
class ExperimentRunner:
    ...
    def run_experiment(self, N: int, M: int, J: int = 5, delta_max: int = 50,
        ...
        Args:
            arm_classes: Pre-defined arm classes (e.g., from Ontario data).
                        If None, uses NHGP synthetic data.
        ...
        # Use provided arm_classes or generate from NHGP
        if arm_classes is not None:
        ...
        for name in ['Whittle', 'MaxAge', 'Myopic', 'WorstState', 'Random']:
            if verbose:
...
class IEEEPlotter:
    ...
    def plot_delta_distribution(self, results: Dict[str, ExperimentResults]) -> plt.Figure:
        ...
        policies = ['Whittle', 'MaxAge', 'Myopic', 'WorstState', 'Random']
        delta_means = [results[p].mean_delta for p in policies if p in results]
        colors = [POLICY_STYLES[p]['color'] for p in policies if p in results]
        policies = [p for p in policies if p in results]  # Filter to available
...
def run_all_experiments(output_dir: str = "results", quick_test: bool = False,
    ...
    het_status = "Heterogeneous p_s [0.3, 0.7]" if heterogeneous else "Homogeneous p_s"

    print("\n" + "=" * 70)
    print("  ROAD DT AoII-ARD RMAB EXPERIMENTS v2.3")
    ...
    if use_ontario:
        print(f"Ontario data dir: {ontario_dir}")
    print("=" * 70 + "\n")

    # Pre-load arm classes
    arm_classes_override = None
    if use_ontario:
        try:
            arm_classes_override = get_ontario_arm_classes(ontario_dir)
            print(f"✅ Ontario data loaded successfully")
        except Exception as e:
            print(f"❌ Failed to load Ontario data: {e}")
            print("   Falling back to NHGP synthetic data")
            use_ontario = False

    # If not using Ontario, create NHGP arm classes with heterogeneous option
    if not use_ontario:
        arm_classes_override = get_nhgp_arm_classes(J=5, R=8, heterogeneous=heterogeneous)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-ontario', action='store_true',
                        help='Use real Ontario pavement data instead of NHGP synthetic data')
    parser.add_argument('--ontario-dir', type=str, default='data/ontario',
                        help='Directory containing Ontario CSV files (ontario_2022.csv, ontario_2023.csv)')
